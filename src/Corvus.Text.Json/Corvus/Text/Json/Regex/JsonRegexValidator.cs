...
namespace Corvus.Text.Json.Internal
{
    ...
    internal ref struct JsonRegexValidator
    {
        ...
        private bool ScanDollar(out JsonRegexNode node)
        {
            if (_pos == _pattern.Length)
            {
                node = CreateNode(JsonRegexNodeKind.One);
            ...
            if (ch is >= '0' and <= '9')
            {
                if (!angled && (_options & JsonRegexOptions.ECMAScript) != 0)
                {
                    int capnum = -1;
                    int newcapnum = ch - '0';
                    _pos++;
            ...
            else if (!angled)
            {
                ...
                switch (ch)
                {
                    ...
                    case '`':
                        capnum = JsonRegexReplacement.LeftPortion;
                        break;

                    case '\'':
                        capnum = JsonRegexReplacement.RightPortion;
                        break;

                    case '+':
                        capnum = JsonRegexReplacement.LastGroup;
                        break;

                    case '_':
                        capnum = JsonRegexReplacement.WholeString;
                        break;
                }
