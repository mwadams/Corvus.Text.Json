...
namespace Corvus.Text.Json
{
    public sealed class JsonSchemaResultsCollector : IJsonSchemaResultsCollector
    {
        ...
        internal static JsonSchemaResultsCollector CreateEmptyInstanceForCaching() => throw new NotImplementedException();
        ...
        private bool EnsureCapacityForResult(bool match)
        {
            ...
            if (_utf8StringBacking.Length - _utf8StringBackingLength < messageLength + 32 + _currentEvaluationPathRange.Length + _currentDocumentEvaluationPathRange.Length + _currentSchemaEvaluationPathRange.Length)
            {
                Enlarge(ref _utf8StringBacking);
            }

            return messageLength > 0;
        }

        private static void Enlarge(ref byte[] backing) => throw new NotImplementedException();
        ...
        private void AppendToSchemaEvaluationPath(JsonSchemaPathProvider path)
        {
            ...
            {
                Enlarge(ref _schemaEvaluationPath);
            }
        ...
        private void AppendToDocumentEvaluationPath(JsonSchemaPathProvider path)
        {
            ...
            {
                Enlarge(ref _documentEvaluationPath);
            }
        ...
        private void UnescapeEncodeAndAppendToDocumentEvaluationPath(ReadOnlySpan<byte> unencodedPropertyName)
        {
        }

        int IJsonSchemaResultsCollector.BeginChildContext(JsonSchemaPathProvider? reducedEvaluationPath, JsonSchemaPathProvider? documentEvaluationPath)
        {
            ...
            if (reducedEvaluationPath is not null)
            {
                AppendToSchemaEvaluationPath(reducedEvaluationPath);
                AppendToEvaluationPath(reducedEvaluationPath);
            }
        ...
        int IJsonSchemaResultsCollector.BeginChildContext<TProviderContext>(TProviderContext providerContext, JsonSchemaPathProvider<TProviderContext>? reducedEvaluationPath, JsonSchemaPathProvider<TProviderContext>? documentEvaluationPath)
        {
            IsConsistent(_sequenceNumber);
            ...
            if (reducedEvaluationPath is not null)
            {
                AppendToSchemaEvaluationPath(providerContext, reducedEvaluationPath);
                AppendToEvaluationPath(providerContext, reducedEvaluationPath);
            }
        ...
        int IJsonSchemaResultsCollector.BeginChildContext(ReadOnlySpan<byte> escapedPropertyName, JsonSchemaPathProvider? reducedEvaluationPath)
        {
            IsConsistent(_sequenceNumber);
        ...
        void IJsonSchemaResultsCollector.CommitChildContext(int sequenceNumber, bool isMatch, JsonSchemaMessageProvider? messageProvider)
        {
            ...
            _currentSchemaEvaluationPathRange = _documentEvaluationPathStack.Pop();
            _sequenceNumber--;
        }
        ...
        private void IsConsistent(int sequenceNumber)
        {
            ...
            Debug.Assert(_evaluationPathStack.Length == _documentEvaluationPathStack.Length && _evaluationPathStack.Length == _schemaEvaluationPathStack.Length);
            Debug.Assert(sequenceNumber == _sequenceNumber);
        }
