...
#pragma warning restore SA1611
    ...
    public static bool PeriodParser(ReadOnlySpan<byte> text, out PeriodBuilder builder)
    {
        ...
        Utf8ValueCursor valueCursor = new(text);
        valueCursor.MoveNext();
        if (valueCursor.Current != (byte)'P')
        {
            return false;
        }
        ...
        while (valueCursor.MoveNext())
        {
            ...
            {
                case (byte)'Y': unit = PeriodUnits.Years; break;
                case (byte)'M': unit = inDate ? PeriodUnits.Months : PeriodUnits.Minutes; break;
                case (byte)'W': unit = PeriodUnits.Weeks; break;
    ...
    /// Sub-second values are normalized to millisecond and "nanosecond within millisecond" values.
