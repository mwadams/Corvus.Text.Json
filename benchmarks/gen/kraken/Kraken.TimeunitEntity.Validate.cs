    ...
    public readonly partial struct TimeunitEntity
    {
        ...
        public ValidationContext Validate(in ValidationContext validationContext, ValidationLevel level = ValidationLevel.Flag)
        {
            ...
            if (level > ValidationLevel.Basic)
            {
                ...
                result = result.PushSchemaLocation("kraken.json#/definitions/https:~1~1www.krakend.io~1schema~1v2.7~1timeunits.json/definitions/timeunit");
            }
                ...
                ValidationLevel level = ValidationLevel.Flag)
            {
                ...
                static bool StringValidator(ReadOnlySpan<char> input, in Corvus.Json.Validate.ValidationContextWrapper context, out ValidationContext result)
                {
                    ...
                    if (Pattern.IsMatch(input))
                    {
                        ...
                        {
                            result = result.WithResult(isValid: true, validationLocationReducedPathModifier: new JsonReference("pattern"), $"Validation pattern - {input.ToString()} matched '^[0-9]+(ns|ms|us|µs|s|m|h)$'");
                        }
                    }
                    else
                    {
                        ...
                        {
                            result = result.WithResult(isValid: false, validationLocationReducedPathModifier: new JsonReference("pattern"), $"Validation pattern - {input.ToString()} did not match '^[0-9]+(ns|ms|us|µs|s|m|h)$'");
                        }
                        else
                        {
                            result = result.WithResult(isValid: false, validationLocationReducedPathModifier: new JsonReference("pattern"), "Validation pattern - The value did not match '^[0-9]+(ns|ms|us|µs|s|m|h)$'");
                        }
                    }
