...
public readonly partial struct NumericFormatHalf
    : IJsonElement<NumericFormatHalf>
{
    private readonly IJsonDocument _parent;
    private readonly int _idx;

    /// <summary>
    /// Initializes a new instance of the <see cref="NumericFormatHalf"/> struct.
    /// </summary>
    /// <param name="value">The value from which to construct the instance.</param>
    internal NumericFormatHalf(IJsonDocument parent, int idx)
    {
        Debug.Assert(idx >= 0);
        _parent = parent;
        _idx = idx;
    ...
    /// </summary>
    public static NumericFormatHalf DefaultInstance { get; }
    ...
    /// </returns>
    public static bool operator ==(in NumericFormatHalf left, in NumericFormatHalf right)
    {
    ...
    /// <c>True</c> if the values are equal.
    /// </returns>
    public static bool operator ==(in NumericFormatHalf left, in JsonElement right)
    {
        return left.Equals(right);
    }
    ...
    public static implicit operator JsonElement(NumericFormatHalf instance)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static NumericFormatHalf From<T>(in T instance)
    ...
    {
        return
            (obj is IJsonElement value && Equals(new NumericFormatHalf(value.ParentDocument, value.ParentDocumentIndex))) ||
...
#if NET
    static NumericFormatHalf IJsonElement<NumericFormatHalf>.CreateInstance(IJsonDocument parentDocument, int parentDocumentIndex) => new(parentDocument, parentDocumentIndex);
#endif

    [DebuggerBrowsable(DebuggerBrowsableState.Never)]
    private string DebuggerDisplay => $"NumericFormatHalf: ValueKind = {ValueKind} : \"{ToString()}\"";
    ...
    {
        return workspace.CreateDocumentBuilder<NumericFormatHalf, Mutable>(this);
    }
...
#if NET
        private Source(Half value) { SimpleTypesBacking.Initialize(ref _simpleTypeBacking, value, static (v, buffer, out written) => v.TryFormat(buffer, out written)); _kind = Kind.NumericSimpleType; }
#endif

        public static implicit operator Source(NumericFormatHalf instance) => new(JsonElement.From(instance));

#if NET
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static implicit operator Source(Half value) => new (value);
#endif
