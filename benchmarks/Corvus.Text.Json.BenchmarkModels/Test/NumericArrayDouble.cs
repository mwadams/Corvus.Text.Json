...
public readonly partial struct NumericArrayDouble
    : IJsonElement<NumericArrayDouble>
{
    private readonly IJsonDocument _parent;
    private readonly int _idx;

    /// <summary>
    /// Initializes a new instance of the <see cref="NumericArrayDouble"/> struct.
    /// </summary>
    /// <param name="value">The value from which to construct the instance.</param>
    internal NumericArrayDouble(IJsonDocument parent, int idx)
    {
        Debug.Assert(idx >= 0);
        _parent = parent;
        _idx = idx;
    ...
    /// </summary>
    public static NumericArrayDouble DefaultInstance { get; }
    ...
    public Test.NumericArrayDouble.ItemsEntity this[int index]
    {
        get
        {
            CheckValidInstance();
            return _parent.GetArrayIndexElement<Test.NumericArrayDouble.ItemsEntity>(_idx, index);
        }
    }
    ...
    /// </summary>
    /// <exception cref="InvalidOperationException">The value is not an array.</exception>
    public ArrayEnumerator<Test.NumericArrayDouble.ItemsEntity> EnumerateArray()
    {
        CheckValidInstance();
        return EnumeratorCreator.CreateArrayEnumerator<Test.NumericArrayDouble.ItemsEntity>(_parent, _idx);
    }
    ...
    /// </returns>
    public static bool operator ==(in NumericArrayDouble left, in NumericArrayDouble right)
    {
    ...
    /// <c>True</c> if the values are equal.
    /// </returns>
    public static bool operator ==(in NumericArrayDouble left, in JsonElement right)
    {
        return left.Equals(right);
    }
    ...
    public static implicit operator JsonElement(NumericArrayDouble instance)
    {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static NumericArrayDouble From<T>(in T instance)
...
#if NET
    static NumericArrayDouble IJsonElement<NumericArrayDouble>.CreateInstance(IJsonDocument parentDocument, int parentDocumentIndex) => new(parentDocument, parentDocumentIndex);
#endif

    [DebuggerBrowsable(DebuggerBrowsableState.Never)]
    private string DebuggerDisplay => $"NumericArrayDouble: ValueKind = {ValueKind} : \"{ToString()}\"";
