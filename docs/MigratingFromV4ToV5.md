...
| **Parsing** | `MyType.Parse(json)` | `ParsedJsonDocument<MyType>.Parse(json)` returning a disposable document, **or** `MyType.ParseValue(json)` for a self-owned copy |
| **Validation** | `entity.Validate(ValidationContext, ValidationLevel)` | `entity.EvaluateSchema()` returning `bool` |
...
> **Type coercion**: Because V5 no longer has a set of common well-known types (`JsonString`, `JsonNumber`, etc.) that all schema types share, you will always need to explicitly coerce values to the specific target type. In V4 this was `sourceInstance.As<TargetType>()`; in V5 it is `TargetType.From(sourceInstance)`. Although slightly more verbose, this makes the intent clearer.
...
V4 uses a `Validate()` method returning a `ValidationContext`. V5 uses `EvaluateSchema()`.
...
`EvaluateSchema()` has multiple overloads for controlling validation level and collecting results:

```csharp
// V5 — detailed validation
bool isValid = v5.EvaluateSchema(ValidationLevel.Detailed, out ValidationContext results);
```
...
```csharp
// V4
MigrationWithDefaults v4 = MigrationWithDefaults.Parse("""{"name":"Jo"}""");
...
MigrationTuple v4 = MigrationTuple.Create(
    (JsonString)"hello",
    MigrationTuple.PrefixItems1Entity.Parse("42"),
    (JsonBoolean)true);
...
### V4: Composition accessors (`AsString`, `AsNumber`, `AsBoolean`)

In V4, extracting the underlying value from a composition type (`oneOf`, `anyOf`, or `allOf`) required going through a composition accessor that returned an intermediate well-known type (`JsonString`, `JsonNumber`, `JsonBoolean`), then casting that to the primitive:
...
### V5: Direct value access on the union type

V5 avoids the need for these intermediate composition accessors. The type itself provides `GetString()`, `TryGetValue()`, and explicit cast operators for the value types implied by its composition entries:
...
### V5: Composition entity types, `TryGetAs*()`, and `Match<TResult>()`

When you need to work with the strongly-typed composition entity rather than just the primitive value, V5 provides `TryGetAs*()` methods and explicit casts to the generated entity types:

```csharp
// V5 — try-get pattern for composition entities
if (v5.TryGetAsOneOf0Entity(out MigrationUnion.OneOf0Entity stringEntity))
{
    string value = (string)stringEntity;
}

// V5 — explicit cast
MigrationUnion.OneOf0Entity stringEntity = (MigrationUnion.OneOf0Entity)v5;
```

V5 adds a `Match` pattern for exhaustive discrimination:

```csharp
// V5 — Match pattern (no V4 equivalent without context)
...
V4 also supports `Match` with a context parameter:

```csharp
// V4 — Match with context
...
> **Note**: V4 uses the well-known types (`JsonString`, `JsonNumber`, `JsonBoolean`) as composition type parameters. V5 uses generated entity types (`OneOf0Entity`, `OneOf1Entity`, `AnyOf0Entity`, `AllOf0Entity`, etc.) instead. The naming follows the keyword and the order of the entries in the schema.
...
```csharp
// V4
MigrationStatusEnum v4 = MigrationStatusEnum.Parse("\"active\"");
...
| `v4 == v4b` | `v5 == v5b` (same) |
| `v4.Validate(ctx, level)` | `v5.EvaluateSchema()` |
...
| `v4.Match(...)` | `v5.Match(...)` (similar — entity types differ) |
...
1. **Update project references** — replace `Corvus.Json` packages with `Corvus.Text.Json` packages
2. **Update namespaces** — replace `using Corvus.Json;` with `using Corvus.Text.Json;`
3. **Wrap parsing in `using`** — ensure all `ParsedJsonDocument<T>` instances are disposed
