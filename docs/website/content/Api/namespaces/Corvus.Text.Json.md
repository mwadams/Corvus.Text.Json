...
Key types include `JsonElement` (the immutable value type), `JsonElement.Mutable` (the mutable builder variant), `JsonElement.Source` (for implicit conversions from .NET primitives), `JsonWorkspace` (pooled memory management), and `Utf8JsonReader`/`Utf8JsonWriter` (high-performance streaming I/O).
