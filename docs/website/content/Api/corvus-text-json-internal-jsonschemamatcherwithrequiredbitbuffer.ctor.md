---
ContentType: "application/vnd.endjin.ssg.content+md"
PublicationStatus: Published
Date: 2026-03-15T00:00:00.0+00:00
Title: "JsonSchemaMatcherWithRequiredBitBuffer Constructors — Corvus.Text.Json.Internal"
---
## Definition

**Namespace:** Corvus.Text.Json.Internal  
**Assembly:** Corvus.Text.Json.dll

## JsonSchemaMatcherWithRequiredBitBuffer {#jsonschemamatcherwithrequiredbitbuffer}

```csharp
public JsonSchemaMatcherWithRequiredBitBuffer(object object, IntPtr method)
```

### Parameters

| Name | Type | Description |
|------|------|-------------|
| `object` | [`object`](https://learn.microsoft.com/dotnet/api/system.object) |  |
| `method` | [`IntPtr`](https://learn.microsoft.com/dotnet/api/system.intptr) |  |

