---
ContentType: "application/vnd.endjin.ssg.content+md"
PublicationStatus: Published
Date: 2026-03-15T00:00:00.0+00:00
Title: "JsonElement.ObjectBuilder.Build.EndInvoke Method — Corvus.Text.Json"
---
## Definition

**Namespace:** Corvus.Text.Json  
**Assembly:** Corvus.Text.Json.dll

## EndInvoke {#endinvoke}

```csharp
public virtual void EndInvoke(ref JsonElement.ObjectBuilder builder, IAsyncResult result)
```

### Parameters

| Name | Type | Description |
|------|------|-------------|
| `builder` | [`ref JsonElement.ObjectBuilder`](/api/corvus-text-json-jsonelement-objectbuilder.html) |  |
| `result` | [`IAsyncResult`](https://learn.microsoft.com/dotnet/api/system.iasyncresult) |  |

