...
public sealed class JsonDocumentBuilderDynamicWriteTests : JsonDomDocumentBuilderDynamicWriteTests
{
    [Fact]
    ...
    protected override JsonDocumentBuilder<JsonElement.Mutable> PrepareDocument(string jsonIn, JsonWorkspace workspace)
    {
        ...
        return jsonDocument.RootElement.BuildDynamicDocument(workspace);
    }
    ...
    protected override void WriteSingleValue(JsonDocumentBuilder<JsonElement.Mutable> document, Utf8JsonWriter writer)
    {
        document.WriteTo(writer);
    }
}

public abstract class JsonDomDocumentBuilderDynamicWriteTests
{
  ...
  ""dinner"": ""pizza""
    ...
    public void WriteCorruptedNumber(string parseJson, string overwriteJson)
    {
        ...
        using (var parsedDocument = ParsedJsonDocument<JsonElement>.Parse(utf8Data))
        ...
        using (var writer = new Utf8JsonWriter(stream))
        {
            ...
            // We build the mutable document having corrupted the source data.
            // This throws because we validate the number during the build
            AssertExtensions.Throws<ArgumentException>(
                "utf8FormattedNumber",
                () => parsedDocument.RootElement.BuildDynamicDocument(workspace).Dispose());
        }
    }
...
",
  ...
  "" boolean "": false
    ...
    public void WriteIncredibleDepth()
    {
        ...
        using (var workspace = JsonWorkspace.Create())
        using (var parsedDoc = ParsedJsonDocument<JsonElement>.Parse(jsonIn, optionsCopy))
...
4
  ...
  "" boolean "": true
    ...
    protected abstract JsonDocumentBuilder<JsonElement.Mutable> PrepareDocument(string jsonIn, JsonWorkspace workspace);

    protected abstract void WriteDocument(JsonDocumentBuilder<JsonElement.Mutable> document, Utf8JsonWriter writer);

    protected abstract void WriteSingleValue(JsonDocumentBuilder<JsonElement.Mutable> document, Utf8JsonWriter writer);
...
public sealed class JsonElementMutableDynamicWriteTests : JsonDomDocumentBuilderDynamicWriteTests
{
    [Fact]
