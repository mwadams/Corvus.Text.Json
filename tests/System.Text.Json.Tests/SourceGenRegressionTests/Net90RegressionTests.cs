...
using Corvus.Text.Json.Tests.SourceGenRegressionTests.Net90;
using Xunit;
using HighLowTemps = Corvus.Text.Json.Tests.SourceGenRegressionTests.Net90.HighLowTemps;
using WeatherForecastWithPOCOs = Corvus.Text.Json.Tests.SourceGenRegressionTests.Net90.WeatherForecastWithPOCOs;

namespace Corvus.Text.Json.Tests.SourceGenRegressionTests
{
    public static class Net90RegressionTests
    {
        [Theory]
        ...
        {
            var ctx = Net90GeneratedContext.Default;
        ...
        {
            string json = JsonSerializer.Serialize(value, Net90GeneratedContext.Default.Options);
            JsonTestHelper.AssertJsonEqual(expectedJson, json);

            T deserializedValue = JsonSerializer.Deserialize<T>(json, Net90GeneratedContext.Default.Options);
            json = JsonSerializer.Serialize(deserializedValue, Net90GeneratedContext.Default.Options);
            JsonTestHelper.AssertJsonEqual(expectedJson, json);
        }
        ...
        public static void UnsupportedType_ThrowsInvalidOperationException()
        {
            ...
            Assert.Throws<InvalidOperationException>(() => JsonSerializer.Serialize(value, value.GetType(), Net90GeneratedContext.Default));
            Assert.Throws<InvalidOperationException>(() => JsonSerializer.Deserialize(json, value.GetType(), Net90GeneratedContext.Default));
        }
        ...
        {
            JsonTypeInfo<HighLowTemps> jsonTypeInfo = Net90GeneratedContext.Default.HighLowTemps;
        ...
        {
            JsonTypeInfo<MyLinkedList> jsonTypeInfo = Net90GeneratedContext.Default.MyLinkedList;
        ...
        public static void CombinedContexts_WorksAsExpected()
        {
            ...
            {
                TypeInfoResolver = JsonTypeInfoResolver.Combine(Net90GeneratedContext.Default, new DefaultJsonTypeInfoResolver())
            };

            // Unlike v6, v7 Contexts do implement IJsonTypeInfoResolver so combined resolvers will produce the expected output.
            string expected = JsonSerializer.Serialize(new HighLowTemps(), Net90GeneratedContext.Default.HighLowTemps);
